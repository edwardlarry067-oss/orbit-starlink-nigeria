...
// POST /api/wallet/credit — add credits (admin only)
router.post("/wallet/credit", adminAuth, async (req, res): Promise<void> => {
  try {
...
// POST /api/wallet/debit — deduct credits (admin only)
router.post("/wallet/debit", adminAuth, async (req, res): Promise<void> => {
  try {
    const { email, amount, description, reference, metadata } = req.body as {
      email: string;
      amount: number;
...
// POST /api/wallet/transfer — transfer credits between wallets (admin only)
router.post("/wallet/transfer", adminAuth, async (req, res): Promise<void> => {
  try {
